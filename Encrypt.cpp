...
string RowEncrpt(string Key, string CinText)
{
	...
	for (int j = 0; j < (CinText.length() / Key.length())+1; j++)
	{
		for (int i = 0; i < Key.length(); i++)
		{
			...
			{
				MatrixOfText[i][j] = CinText[j*Key.length() + i];
			}
		}
	}
	for (int j = 0; j < Key.length(); j++)
	{
		...
		{
			PrinText += MatrixOfText[keyrow[j]][i];
		}
	}
...
string Rail_fence(string Key, string CinText)
{
	...
	for (int i = 0; i < RailSize; i++)
	{
		for (int j = 0; j < CinText.length(); j++)
		{
			if (Rail[i][j] != '\n')
			{
				PlainText.push_back(toupper(Rail[i][j]));
			}
		}
	}
	return PlainText;
